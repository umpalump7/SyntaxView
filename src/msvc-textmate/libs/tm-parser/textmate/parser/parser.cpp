    ...
    bool firstLine, char const* gpos)
{
    ...
    for (auto const& it : rules) {
        ...
        if (!rule->children.empty()) {
            ...
            ++scopes.tracking;
            parse(m.buffer(), m.buffer() + to, stack, scopes, firstLine, from, gpos);
...
static size_t apply_rules(size_t rank, std::vector<rule_t*> const& rules,
    ...
    std::map<size_t, regexp::match_t>& match_cache, char const* gpos)
{
    ...
    std::map<size_t, regexp::match_t>& match_cache, char const* gpos)
{
    ...
    rank = apply_rules(rank, rules, first, last, options, i, res, match_cache, gpos);
    ...
    if (stack->end_pattern) {
        ...
        if (regexp::match_t const& match = regexp::search(
                stack->end_pattern, first, last, first + i, last, gpos, options))
    ...
    scopes_t& scopes, bool firstLine, size_t i, char const* gpos)
{
    ...
    while (it != while_rules.end()) {
        if (regexp::match_t const& m = regexp::search((*it)->while_pattern, first, last, first + i, last, gpos)) {
    ...
    collect_rules(first, last, i, firstLine, stack, rules, match_cache, gpos);
    ...
    while (!rules.empty()) {
        ...
        if (m.match.begin() < i) {
            ...
            if (m.match = regexp::search(
                    ptrn, first, last, first + i, last, gpos,
        ...
        } else // regular match-rule
        {
            ...
            apply_captures(scope, m.match, rule->captures, scopes, firstLine, gpos);
            if (m.match = regexp::search(
                    m.rule->match_pattern, first, last, first + i, last, gpos,
                    anchor_options(firstLine, stack->anchor == i, first, last)))
                rules.insert(m);
...
__declspec(dllexport) stack_ptr parse(char const* first, char const* last, stack_ptr stack,
    std::map<size_t, scope::scope_t>& map, bool firstLine)
{
    ...
    auto res = parse(first, last, stack, scopes, firstLine, 0, first-1);
