...
#define DISABLE_ADD_GRAMMAR_THREADS 1
...
__declspec(dllexport) stack_ptr grammar_t::seed() const
{
    return std::make_shared<stack_t>(_rule.get(),
        _rule ? _rule->scope_string : "");
}

__declspec(dllexport) grammar_ptr parse_grammar(Json::Value const& json)
{
    PatternErrors = "";
