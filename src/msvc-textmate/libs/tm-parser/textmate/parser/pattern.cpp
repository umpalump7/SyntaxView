...
void pattern_t::init(std::string const& pattern, OnigOptionType options)
{
    ...
    // TODO robit iba v pripade ak pouzite utf16
    wpattern = std::wstring(pattern.begin(), pattern.end());
    
    int r = onig_new(&tmp, (OnigUChar const*)wpattern.data(),
        (OnigUChar const*)wpattern.data() + wpattern.size()*2, options,
    ...
    if (r == ONIG_NORMAL) {
        ...
        // pattern.c_str());
        PatternErrors += std::string(s, s + strlen((char const*)s)) + " (" + pattern + ")\n";
        if (tmp)
            onig_free(tmp);
    }
}
...
match_t search(pattern_t const& ptrn, char const* first, char const* last,
    char const* from, char const* to, char const* gpos, OnigOptionType options)
{
    if (ptrn) {
