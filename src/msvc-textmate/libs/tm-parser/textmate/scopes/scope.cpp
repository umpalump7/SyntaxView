...
__declspec(dllexport) scope_t::scope_t() { ; }
__declspec(dllexport) scope_t::scope_t(char const* scope) { setup(scope); }
__declspec(dllexport) scope_t::scope_t(std::string const& scope) { setup(scope); }

__declspec(dllexport) scope_t::scope_t(scope_t::node_t* node)
    : node(node)
{
    if (node)
        node->retain();
...
__declspec(dllexport) scope_t::scope_t(scope_t&& rhs) { std::swap(node, rhs.node); }

__declspec(dllexport) scope_t::scope_t(scope_t const& rhs)
{
    if ((node = rhs.node))
        node->retain();
}
...
__declspec(dllexport) std::string const& scope_t::back() const
{
    // ASSERT(node);
    return node->_atoms;
}

__declspec(dllexport) size_t scope_t::size() const
{
    if (!node) {
        return 0;
    }
...
__declspec(dllexport) std::string to_s(scope_t const& s) { return (std::string)s; }
