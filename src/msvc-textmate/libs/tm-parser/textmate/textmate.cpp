...
std::vector<list_item_t> Textmate::theme_extensions()
{
  ...
  for(auto ext : extensions) {
      ...
      for (int i = 0; i < themes.size(); i++) {
            list_item_t item = {
                item.name = ext.package["id"].asString(),
                item.description = ext.package["description"].asString(),
                item.value = ext.path + "/" + themes[i]["path"].asString()
            };
        ...
        for (int i = 0; i < langs.size(); i++) {
            ...
            if (lang.isMember("extensions")) {
                ...
                for (int j = 0; j < exts.size(); j++) {
                    ...
                    list_item_t item = {
                        item.name = lang["id"].asString(),
                        item.description = ext.package["description"].asString(),
                        item.value = ex.asString()
                    };
                    res.push_back(item);
                    break;
...
Textmate::run_highlighter(char *_text, language_info_ptr lang, theme_ptr theme,
  ...
  while (it != scopes.end()) {
    ...
    span_info_t span = {span.start = (int16_t)n,
                        span.length = (int16_t)(l - n),
                        span.fg =
                            {
                        ...
                        span.bg = {0, 0, 0, 0},
                        span.bold = style.bold == bool_true,
                        span.italic = style.italic == bool_true,
                        span.underline = style.underlined == bool_true,
                        // .state = state,
                        span.scope = scopeName};
