...
struct theme_t {
    ...
    __declspec(dllexport) color_info_t foreground() const;
    __declspec(dllexport) color_info_t background(std::string const& fileType = NULL_STR) const;

   __declspec(dllexport) style_t const& styles_for_scope(scope::scope_t const& scope);
...
__declspec(dllexport) theme_ptr parse_theme_mod(Json::Value& json, std::string& fileName);
