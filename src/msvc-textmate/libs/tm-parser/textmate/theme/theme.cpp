...
void theme_t::shared_styles_t::setup_styles(Json::Value const& themeItem)
{
    ...
    if (themeItem.isMember("settings")) {
        ...
        get_settings_color(themeItem["colors"]["editor.foreground"], &default_style.foreground);
        get_settings_color(themeItem["colors"]["editor.background"], &default_style.background);
    ...
    for (int i = 0; i < settings.size(); i++) {
        ...
        if (!item.isMember("scope") && !has_default_style) {
            item["scope"] = "default";
            has_default_style = true;
            default_style = parse_styles(item, "default");
            //continue; // todo revise
        }

        //Json::Value scope = settings[i]["scope"];
        Json::Value scope = item["scope"];
    ...
    if (has_default_style) {
        _foreground = default_style.foreground;
        _background = default_style.background;
    }
}
...
__declspec(dllexport) color_info_t theme_t::foreground() const
{
    return _styles->_foreground;
}

__declspec(dllexport) color_info_t theme_t::background(std::string const& fileType) const
{
...
{
    std::string t;
    return parse_theme_mod(themeItem, t);
}

__declspec(dllexport) theme_ptr parse_theme_mod(Json::Value& themeItem, std::string& fileName)
{
    // std::cout << themeItem << std::endl;
    static std::map<std::string, theme_ptr> Cache;
    std::string uuid = themeItem["uuid"].asString();
...
__declspec(dllexport) style_t const& theme_t::styles_for_scope(scope::scope_t const& scope)
{
